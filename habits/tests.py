...
from habits.services import calculate_reminder_time
...
class HabitTestCase(APITestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(email="test@sky.pro", tg_id="123456789")  # Добавим tg_id для тестов
        self.user2 = User.objects.create(email="other@sky.pro", tg_id="987654321")  # Для проверки доступа
    ...
    def test_habit_list(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(data["count"], 1)  # Должна быть только 1 привычка текущего пользователя
        self.assertEqual(data["results"][0]["action"], self.habit.action)
        self.assertEqual(data["results"][0]["owner"], self.user.pk)
        self.assertEqual(data["results"][0]["reminder_task_id"], self.habit.reminder_task_id)
    ...
    def test_habit_update_with_reminder(self, mock_revoke, mock_apply_async_method):
        ...
        mock_apply_async_method.assert_called_once_with(
            args=[self.habit.pk],
            eta=mock_apply_async_method.call_args[1]['eta']
        )
        self.assertIsInstance(mock_apply_async_method.call_args[1]['eta'], timezone.datetime)
